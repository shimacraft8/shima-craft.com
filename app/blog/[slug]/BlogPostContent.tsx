...
/**
 * 記事詳細の描画本体。/blog/[slug]（静的生成・公開用）と
 * /blog/[slug]/preview（動的・下書きプレビュー用）の両方から呼ばれる。
 *
 * generateStaticParams + searchParams を同じルートで併用すると、Next.jsの
 * 仕様上そのルートが常に動的レンダリングへ倒れ、Cloudflare(OpenNext)上では
 * DYNAMIC_SERVER_USAGE エラーになる（vercel/next.js#49735、OpenNext公式の
 * 推奨対応も「searchParamsが必要なページはルートを分離する」）。
 * そのためdraftKeyの有無でルート自体を分け、このファイルはsearchParamsを
 * 一切受け取らない共通ロジックとして両ルートから呼び出す。
 */
...
export async function buildBlogPostMetadata(
  ...
  const post = await getBlogPostBySlug(slug, draftKey);
