...
const NOTES = [
  "写真の処理はお使いの端末内（ブラウザの中）で行われます。会員の方は色解析の精度向上のため写真の縮小版（256×256px）をSHIMA CRAFTサーバー経由でGroq AI（Llama 4）に送信します。写真はサーバーに保存されません。",
...
const FAQS = [
  ...
  {
    q: "写真はどこかへ送信・保存されますか？",
    a: "写真の処理は基本的にお使いの端末内（ブラウザの中）で行います。会員の方は色解析の精度向上のため、写真の縮小版（256×256px）をSHIMA CRAFTサーバー経由でGroq AI（Meta Llama 4）に送信します。写真はサーバーに保存されません。SHIMA CRAFTが写真をAI学習・広告へ利用することもありません。結果画像は画面上で保存でき、保存しない場合はページを離れると再表示できません。",
  },
  {
    q: "元の色を再現できますか？",
